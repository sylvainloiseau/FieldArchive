...
Linguistic fieldwork data consist of a variety of objects: texts, maps, recordings, stimulus sets, still images, specimens of various kinds of realia, information about people, places, events. This data is a dense web of relations: between people and data session they participate in, or recording or photo they are featured in, between event and produced documents and recordings, between event and photo and places, etc.

Linguists have little solution for the description of this data, that would make them 1/ more searchable and discoverable on one hand, and 2/ more archivable for future use and future research questions on the other hand, whereas this contextualisation of the linguistic material proper could turn out, in the futur, to be the only remaining data about the language and be precious for authorizing new kind of research, as as stressed by the documentary linguistic perspective.
...
- **Internal data source**: annotation (RDF triples) that are entered directly into the application and can be modified
- **External data source**: annotation (RDF triples) that are imported from a file produced by a third-party tool (Tropy, Lameta, Gramps). These sources are read-only in the application and can be re-imported at any time
...
Entities can have one or more RDF types: `rico:Person`, (in the RICO ontology), `foaf:Person` (in the FOAF ontology), maximum one by ontology. The **Type navigator**, on the left, allows you to see the types grouped by ontology. When you click on a type, you can see the entities that have that type in the table on the right.
